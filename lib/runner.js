 ...
 */
Runner.prototype.fGetSeleniumWebdriverModule = function () {
    return webdriver;
};
 ...
 * @public
 */
Runner.prototype.run = function( theACVResetter /* ACV OJO 201410262213 to reset info held from previous protractor Runner and jasmine executions in same node js process. */) {
  var self = this,
      driver,
      specs,
      testResult;
  ...
  if (!specs.length) {
    ...
    if( theACVResetter && ( typeof theACVResetter == "function")) { /* ACV OJO 201410262213 to reset info held from previous protractor Runner and jasmine executions in same node js process. */
        theACVResetter( "Runner", this, "Runner.preparers_");
    }
  ...
  return this.driverprovider_.setupEnv().then(function() {
    ...
    var aFramework = require(frameworkPath); /* ACV OJO 201410262213 to reset info held from previous protractor Runner and jasmine executions in same node js process. */

    aFramework.run(self, specs, function(result) { /* ACV OJO 201410262213 to reset info held from previous protractor Runner and jasmine executions in same node js process. */
      deferred.resolve(result);
    }, theACVResetter); /* ACV OJO 201410262213 to reset info held from previous protractor Runner and jasmine executions in same node js process. */
